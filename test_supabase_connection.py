...
def test_supabase_connection():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            data = response.json()
            print(f"   Schema info: {data}")
        else:
            print(f"⚠️  Schema query failed: {response.status_code}")
            print("   This is normal for a new project")
            
    except Exception as e:
        print(f"❌ Schema test error: {e}")
    
    # Test 4: Test inserting a sample record
    ...
    print("3. Run the SQL script from supabase_migration.py")
