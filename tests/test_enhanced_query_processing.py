...
async def test_enhanced_query_processing():
    ...
    test_queries = [
        ...
        },
        {
            ...
            "expected_filters": ["indication", "primary_drug", "trial_status", "geography"],
    ...
    try:
        ...
        for i, test_case in enumerate(test_queries, 1):
            ...
            try:
                parsed_result = await server._parse_natural_language_query(test_case['query'])
        ...
        if successful_tests:
            avg_llm_confidence = sum(r['llm_confidence'] for r in successful_tests) / len(successful_tests)
            avg_reasoning_confidence = sum(r['reasoning_confidence'] for r in successful_tests) / len(successful_tests)
            
            print(f"\nAverage LLM Confidence: {avg_llm_confidence:.2f}")
            print(f"Average Reasoning Confidence: {avg_reasoning_confidence:.2f}")
            
            # Filter accuracy
            total_expected = sum(len(test_case['expected_filters']) for test_case in test_queries)
            ...
            print(f"  Accuracy: {(total_found - total_missing)/total_expected*100:.1f}%")
